...
function closeSpelling(a: string, b: string): boolean {
  ...
  const maxEd = min <= 6 ? 1 : 2;
  return editDistance(a, b) <= maxEd;
}

/**
 * True when two people names refer to the same manager.
 * Accepts case, punctuation, token order, compacted names,
 * aliases, and small spelling differences.
 */
export function namesMatchForClaim(a: string, b: string): boolean {
  ...
  if (closeSpelling(ca, cb)) return true;
...
function strippedTeamKey(value: string): string {
...
export function teamNamesMatch(a: string, b: string): boolean {
  ...
  const sa = strippedTeamKey(a);
  const sb = strippedTeamKey(b);
...
function closeTeamMatch(row: LeagueRosterRow, typed: string): boolean {
  ...
  return closeSpelling(strippedTeamKey(row.teamName), strippedTeamKey(typed));
}
...
export function validateClaimInputs(args: {
  ...
  const teamName = args.teamName.trim();
  const entryId =
    args.entryId ?? parseEntryId(teamName) ?? parseEntryId(fullName);
  if (!fullName && !teamName && entryId == null) {
    return "Enter your FPL team name, the name from the standings, or both.";
  }
  if (
    fullName &&
    !canonicalKeyFromName(fullName) &&
    parseEntryId(fullName) == null
  ) {
  ...
  if (input.fullName && input.teamName) {
    ...
    if (nameHits.length === 0 && teamHits.length === 0) {
      return {
        kind: "error",
        message:
          "No manager in this league matched that name or team. Check the spelling on the standings and try again.",
      };
    }
    return {
      kind: "error",
      message:
        "Couldn't uniquely match that name and team. Try the other field, or paste your FPL entry ID from fantasy.premierleague.com/entry/XXXXXX/.",
    };
  }
  ...
  if (input.fullName) {
    ...
    if (nameHits.length === 0) {
      return {
        kind: "error",
        message:
          "No manager in this league matched that name. Use the name from the standings, or add your FPL team name.",
      };
    }
    return {
      kind: "error",
      message:
        "Several managers match that name. Add your FPL team name to narrow it down.",
    };
  }
  ...
  if (input.teamName) {
    ...
    if (teamHits.length === 0) {
      return {
        kind: "error",
        message:
          "No FPL team in this league matched that name. Copy it from the league table, or add your manager name.",
      };
    }
    return {
      kind: "error",
      message:
        "Several teams match that name. Add your name from the standings to narrow it down.",
    };
  }

  return {
    kind: "error",
    message: "Enter your FPL team name, the name from the standings, or both.",
  };
}
