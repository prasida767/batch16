...
import { afterAuthPath, isPublicPath, safeNextPath } from "@/lib/auth/paths";
...
describe("claim matching", () => {
  ...
  it("matches names by case, spaces, order, and compact form", () => {
    expect(namesMatchForClaim("Abhishek Gupta", "abhishek gupta")).toBe(true);
    expect(namesMatchForClaim("Abhishek Gupta", "  Abhishek   Gupta  ")).toBe(
      true,
    );
  ...
  it("allows small spelling differences in names", () => {
    expect(namesMatchForClaim("Abhishek Gupta", "Abhisek Gupta")).toBe(true);
    expect(namesMatchForClaim("Prasiddha Khadka", "Prasidha Khadka")).toBe(
      true,
    );
  });
...
describe("league roster matching", () => {
  ...
  it("ignores extra spaces and case on team names", () => {
    const result = matchLeagueRoster(roster, {
      fullName: "",
      teamName: "  BATCH   united  ",
      entryId: null,
    });
    expect(result.kind).toBe("ok");
    if (result.kind === "ok") expect(result.row.entryId).toBe(111);
  });

  it("allows a small spelling difference in the team name", () => {
    const result = matchLeagueRoster(roster, {
      fullName: "",
      teamName: "Batch Untied",
      entryId: null,
    });
    expect(result.kind).toBe("ok");
    if (result.kind === "ok") expect(result.row.entryId).toBe(111);
  ...
  it("fails with a clear message when nothing matches", () => {
    ...
    if (unknown.kind === "error") {
      expect(unknown.message).toMatch(/no manager in this league/i);
    }
  });
