  ...
  try {
    ...
    if (live.length === 0) {
      return {
        kind: "unavailable",
        message:
          "FPL hasn't published this league's managers yet. Try again after the season starts, or paste your FPL entry ID later.",
      };
    }
    ...
    return {
      kind: "unavailable",
      message:
        "Couldn't load the FPL league right now. Wait a minute and try again, or paste your FPL entry ID from your team URL.",
    };
  }
}
...
async function ensureManagerForEntry(row: LeagueRosterRow): Promise<{
  ...
  const key =
    canonicalKeyFromName(row.playerName) || `entry_${row.entryId}`;
...
/**
 * Match a league manager against live FPL standings (name, team, and/or entry ID).
 */
...
> {
  ...
  if (live.kind === "ok") {
    ...
    if (input.teamName && input.entryId == null) {
      return {
        kind: "error",
        message: `${live.message} Team names can't be checked until FPL loads — paste your FPL entry ID instead.`,
      };
    }
  } else {
    return { kind: "error", message: live.message };
  ...
  if (claimed) {
    return {
      kind: "error",
      message: "That manager is already linked to another account. Ask an admin to unlink it if it's yours.",
    };
  }
